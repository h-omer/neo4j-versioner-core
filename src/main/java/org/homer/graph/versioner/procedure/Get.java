...
public class Get {
    ...
    public Stream<PathOutput> getCurrentPath(
		...
		Optional.ofNullable(entity.getSingleRelationship(RelationshipType.withName(CURRENT_TYPE), Direction.OUTGOING)).map(builder::addOutgoingRelationship);
    ...
    public Stream<NodeOutput> getCurrentState(
        ...
        return Stream.of(Optional.ofNullable(entity.getSingleRelationship(RelationshipType.withName(CURRENT_TYPE), Direction.OUTGOING))
    ...
    public Stream<PathOutput> getAllState(
		...
		builder.addOutgoingRelationship(entity.getSingleRelationship(RelationshipType.withName(CURRENT_TYPE), Direction.OUTGOING));
		StreamSupport.stream(entity.getRelationships(RelationshipType.withName(HAS_STATE_TYPE), Direction.OUTGOING).spliterator(), false)
				//.sorted((a, b) -> -1 * Long.compare((long)a.getProperty(START_DATE_PROP), (long)b.getProperty(START_DATE_PROP)))
				.forEach(rel ->
					Optional.ofNullable(rel.getEndNode().getSingleRelationship(RelationshipType.withName(PREVIOUS_TYPE), Direction.OUTGOING))
    ...
    public Stream<NodeOutput> getAllStateByLabel(
		...
		return StreamSupport.stream(entity.getRelationships(RelationshipType.withName(HAS_STATE_TYPE), Direction.OUTGOING).spliterator(), false)
    ...
    public Stream<NodeOutput> getStateByDate(
		...
		return StreamSupport.stream(entity.getRelationships(RelationshipType.withName(HAS_STATE_TYPE), Direction.OUTGOING).spliterator(), false)
				.filter(relationship -> relationship.getProperty(START_DATE_PROP).equals(date))
				.map(Relationship::getEndNode)
				.map(NodeOutput::new);
    }
}
