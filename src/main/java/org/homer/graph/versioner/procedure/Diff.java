...
import org.homer.graph.versioner.output.DiffOutput;
...
public class Diff {
    ...
    public Stream<DiffOutput> diffFromPrevious(
		...
		Optional<Node> stateFrom = Optional.ofNullable(state.getSingleRelationship(RelationshipType.withName(PREVIOUS_TYPE), Direction.OUTGOING)).map(Relationship::getEndNode);
	...
	public Stream<DiffOutput> diffFromCurrent(
		...
		Optional<Node> currentState = Optional.ofNullable(state.getSingleRelationship(RelationshipType.withName(HAS_STATE_TYPE), Direction.INCOMING))
				.map(Relationship::getStartNode).map(entity -> entity.getSingleRelationship(RelationshipType.withName(CURRENT_TYPE), Direction.OUTGOING))
	...
	private Stream<DiffOutput> diffBetweenStates(Optional<Node> from, Optional<Node> to) {
		...
		propertiesFrom.forEach((key, value) -> {
			...
			String operation = foundValue.map(val -> compareObj(val, value) ? "" : DIFF_OPERATION_UPDATE).orElse(DIFF_OPERATION_REMOVE);
		...
		propertiesTo.forEach((key, value) -> {
			if(!propertiesFrom.containsKey(key)) {
				diffs.add(new DiffOutput(DIFF_OPERATION_ADD, key, null, value));
			}
		});
