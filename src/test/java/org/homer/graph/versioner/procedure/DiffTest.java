...
public class DiffTest {
	...
	@Test public void shouldDiffGetTheDiffBetweenTwoStatesCorrectly() {
		...
		try (Driver driver = GraphDatabase.driver(neo4j.boltURI(), Config.build().withEncryption().toConfig()); Session session = driver.session()) {
			...
			// Then
			assertThat(result.next().values().stream().map(Value::asString).collect(Collectors.toList()), contains(DIFF_OPERATION_REMOVE, "delete", "delete", "null"));
			assertThat(result.next().values().stream().map(Value::asString).collect(Collectors.toList()), contains(DIFF_OPERATION_UPDATE, "update", "old", "new"));
			assertThat(result.next().values().stream().map(Value::asString).collect(Collectors.toList()), contains(DIFF_OPERATION_ADD, "new", "null", "new"));
	...
	@Test public void shouldDiffGetTheDiffBetweenTwoStatesCorrectlyWithDifferentObjectTypes() {
		...
		try (Driver driver = GraphDatabase.driver(neo4j.boltURI(), Config.build().withEncryption().toConfig()); Session session = driver.session()) {
			...
			// Then
			assertThat(result.next().values().stream().map(value -> value.asObject().toString()).collect(Collectors.toList()), contains(DIFF_OPERATION_UPDATE, "a", "1", "2"));
			assertThat(result.next().values().stream().map(value -> value.asObject().toString()).collect(Collectors.toList()), contains(DIFF_OPERATION_UPDATE, "c", "stringA", "stringB"));
			assertThat(result.next().values().stream().map(value -> value.asObject().toString()).collect(Collectors.toList()), contains(DIFF_OPERATION_UPDATE, "f", "[1, 2]", "[1, 3]"));
			assertThat(result.hasNext(), is(false));
		}
	}
